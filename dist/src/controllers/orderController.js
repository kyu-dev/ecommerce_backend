...
async function createOrderForUser(userId) {
    ...
    for (const item of cart.items) {
        ...
        if (item.quantity > actualStock) {
            throw new Error(`Stock insuffisant pour le produit "${item.product.name}". Stock actuel : ${actualStock}, demandé : ${item.quantity}.`);
        }
    }
    // Calcul du total
    ...
    await prismaClient_1.default.cartItem.deleteMany({
...
const createCheckoutSession = async (req, res) => {
    ...
    try {
        ...
        const session = await stripe_1.default.checkout.sessions.create({
            ...
            line_items,
            mode: "payment",
            success_url: (process.env.FRONTEND_URL || "http://localhost:4000") + "/success",
